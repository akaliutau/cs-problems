 ...
 * is 2 + 4 = 6. 
 * After adding -4 to A[0], the array is [-2,-1,3,4], and the sum
 * of even values is -2 + 4 = 2. 
 * After adding 2 to A[3], the array is
...
public class SumOfEvenNumbers {
	...
	public static int[] sumEvenAfterQueries(int[] a, int[][] queries) {
		...
		int[] res = new int[q];
		int sum = Arrays.stream(base)
				.filter(SumOfEvenNumbers::even)
				.sum();
		for (int i = 0; i < q; i++) {
			int[] query = queries[i];
			int before = base[query[1]];
			base[query[1]] = before + query[0];
			int after = base[query[1]];
			if (before % 2 == 0) {
				sum -= before;
			}
			if (after % 2 == 0) {
	...
	public static void main(String[] arg) {
		...
		Utils.print(sumEvenAfterQueries(a, queries));
