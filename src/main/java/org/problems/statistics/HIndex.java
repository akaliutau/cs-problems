 ...
 * 
 * According to the definition of h-index: "A scientist has index h if h of
 ...
 * 
 * Explanation: [3,0,6,1,5] means the
...
public class HIndex {
	...
	public static int hIndex(int[] citations) {
		...
		if (citations[0] >= total){
			return total;
        }
		for (int idx = 1; idx < total; idx++) {
			if (citations[idx] >= total - idx && citations[idx - 1] <= total - idx){
				return total - idx;
            }
		}
