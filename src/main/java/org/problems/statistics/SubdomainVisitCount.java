 ...
 * discuss.leetcode.com", "9001 leetcode.com", "9001 com"]
 * 
...
public class SubdomainVisitCount {
	...
	static class Domain {
		...
		public String toString() {
			return String.format("%d %s", counter, name);
		}
		
		
	}

	static List<String> getSubDomains(String str) {
		...
		for (int i = 0; i < n; i++) {
			result.add(String.join(".", res[i]).replace("null.", ""));
		}	
		return result;
	}
	...
	public static List<String> subdomainVisits(String[] cpdomains) {
		...
		for (String domain : cpdomains) {
			...
			List<String> subs = getSubDomains(parts[1]);

			for (String sub : subs) {
				map.putIfAbsent(sub, new Domain(sub));
				map.get(sub).counter += count;
			}
		}

		return map.values().stream().map(Domain::toString).collect(Collectors.toList());
