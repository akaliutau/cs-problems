...
public class CanPlaceFlowers {
	...
	static boolean adjacentExist(int pos, int[] flowerbed) {
		if (pos - 1 >= 0 && flowerbed[pos-1] == 1) {
			return true;
		}
		if (pos + 1 < flowerbed.length && flowerbed[pos+1] == 1) {
			return true;
		}
		return false;
	}
	...
	public static boolean canPlaceFlowers(int[] flowerbed, int n) {
        ...
        for (int i = 0; i < flowerbed.length; i++) {
        	if (flowerbed[i] == 0 && !adjacentExist(i,flowerbed)) {
        		flowerbed[i] = 1;
        		n--;
        		if (n == 0) {
        			return true;
